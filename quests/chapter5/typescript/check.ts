import { dfs, bfs, shortestPath, hasCycle } from "./graphs";
...
export function runCheck(): boolean {
  ...
  if (dfsResult.length !== 5 || !dfsResult.includes(1) || !dfsResult.includes(2)) {
    console.error(`❌ DFS failed. Expected to visit all 5 nodes, got: ${JSON.stringify(dfsResult)}`);
    passed = false;
  } else {
  ...
  if (dfsResult2.length !== 2) {
    console.error(`❌ DFS should only visit connected nodes. Expected 2, got: ${dfsResult2.length}`);
    passed = false;
  } else {
  ...
  const graphWithCycle: Graph = new Map([
    [1, [2]],
    [2, [3]],
    [3, [1]],
  ]);
  ...
  const graphNoCycle: Graph = new Map([
    [1, [2, 3]],
    [2, [4]],
    [3, [4]],
    [4, [5]],
  ...
  const graphSelfLoop: Graph = new Map([
    [1, [1, 2]],
    [2, []],
  ]);
