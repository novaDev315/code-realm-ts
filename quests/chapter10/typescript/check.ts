import { hashPassword, validateJWT, rateLimit, sanitizeInput } from "./security";
...
export function runCheck(): boolean {
  ...
  const hashCases: TestCase<{ password: string; salt: string }, boolean>[] = [
  ...
  for (const test of jwtTests) {
    ...
    if (isValid !== expectedValid) {
      console.error(`validateJWT("${test.input.substring(0, 20)}...") expected valid=${expectedValid}, got ${isValid}`);
      passed = false;
    } else {
  ...
  const sanitizeCases: TestCase<string, string>[] = [
