import { DistributedSystem, designScalableAPI, optimizeDatabase, SystemRequest, SystemResponse } from "./architect";
...
export function runCheck(): boolean {
  ...
  try {
    ...
    const highPriority: SystemRequest = {
      id: "high-priority",
      data: { urgent: true },
      priority: 10
    };

    const lowPriority: SystemRequest = {
      id: "low-priority",
      data: { urgent: false },
      priority: 1
    };
  ...
  if (passed) {
    ...
    console.log("\n❌ Some tests failed. Review the SOLUTIONS.ts for reference implementation.");
    process.exit(1);
  }
